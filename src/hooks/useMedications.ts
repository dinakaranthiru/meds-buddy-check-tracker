...
export function useAddMedication() {
  ...
  return useMutation({
    ...
    onMutate: async (newMed: NewMedication) => {
      ...
      await queryClient.cancelQueries({ queryKey: ['medications', currentUserId] });

      // Snapshot the previous value
      const previousMedications = queryClient.getQueryData<Medication[]>(['medications', currentUserId]);

      // Optimistically update to the new value
      queryClient.setQueryData<Medication[]>(['medications', currentUserId], (old) => {
    ...
    onError: (err, newMed, context) => {
      ...
      queryClient.setQueryData(['medications', currentUserId], context?.previousMedications);
    ...
    onSettled: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ['medications', currentUserId] });
    },
  });
}